...
class YoloDatasets(keras.utils.Sequence):
    ...
    def get_random_data(self, annotation_line, input_shape, max_boxes=500, jitter=.3, hue=.1, sat=0.7, val=0.4, random=True):
        ...
        #------------------------------#
        # Read the image and convert it to RGB image
        #------------------------------#
    ...
    def get_random_data_with_Mosaic(self, annotation_line, input_shape, max_boxes=500, jitter=0.3, hue=.1, sat=0.7, val=0.4):
        ...
        #---------------------------------#
        # Split the images and put them together
        #---------------------------------#
        ...
        #---------------------------------#
        # Further process bounding boxes
        #---------------------------------#
    ...
    def preprocess_true_boxes(self, true_boxes, input_shape, anchors, num_classes):
        ...
        #-----------------------------------------------------------#
        # Get box coordinates and image size
        #-----------------------------------------------------------#
        true_boxes  = np.array(true_boxes, dtype='float32')
        input_shape = np.array(input_shape, dtype='int32')
        
        #-----------------------------------------------------------#
        # There are three feature layers in total
        #-----------------------------------------------------------#
        num_layers  = len(self.anchors_mask)
        #-----------------------------------------------------------#
        # m is number of images, grid_shapes are grid shapes
        #-----------------------------------------------------------#
        m           = true_boxes.shape[0]
        grid_shapes = [input_shape // {0:32, 1:16, 2:8}[l] for l in range(num_layers)]
        #-----------------------------------------------------------#
        # y_true format: (m,13,13,3,85)(m,26,26,3,85)(m,52,52,3,85)
        #-----------------------------------------------------------#
        ...
        #-----------------------------------------------------------#
        # Calculate center and width/height of true boxes
        # Center point (m,n,2), width/height (m,n,2)
        #-----------------------------------------------------------#
        boxes_xy = (true_boxes[..., 0:2] + true_boxes[..., 2:4]) // 2
        boxes_wh =  true_boxes[..., 2:4] - true_boxes[..., 0:2]
        #-----------------------------------------------------------#
        # Normalize true boxes to decimal form
        #-----------------------------------------------------------#
        ...
        #-----------------------------------------------------------#
        # Only valid when width and height are greater than 0
        #-----------------------------------------------------------#
        ...
        for b in range(m):
            ...
            for t, ratio in enumerate(max_ratios):
                ...
                #-----------------------------------------------------------#
                #   Find which feature layer each true box belongs to
                #-----------------------------------------------------------#
                for l in range(num_layers):
                    for k, n in enumerate(self.anchors_mask[l]):
                        if not over_threshold[n]:
                            continue
                        #-----------------------------------------------------------#
                        #   floor is used for rounding down to find the x,y coordinates of the grid cell that the true box belongs to
                        #-----------------------------------------------------------#
                        ...
                        for offset in offsets:
                            ...
                            if box_best_ratios[l][b, local_j, local_i, k] != 0:
                                if box_best_ratios[l][b, local_j, local_i, k] > ratio[n]:
                                    ...
                                    continue
                            #-----------------------------------------------------------#
                            # c represents the class of the current true box
                            #-----------------------------------------------------------#
                            c = true_boxes[b, t, 4].astype('int32')
                            #-----------------------------------------------------------#
                            #   y_true shape: (m,13,13,3,85)(m,26,26,3,85)(m,52,52,3,85)
                            #   The last dimension 85 can be split into 4+1+80:
                            #   4 for box center and width/height,
                            #   1 for confidence score,
                            #   80 for class probabilities
                            #-----------------------------------------------------------#
